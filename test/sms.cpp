...
BOOL gsmInit()
{
	...
	WriteComm("AT\r", 3);
	Sleep(500);
	ReadComm(ans, ansLength);
	if (strstr(ans, "OK") == NULL)  
		return FALSE;

	// ECHO OFF  DEBUG_ERROR: ATE is used to off echo, ATE1 used to on echo
	WriteComm("ATE1\r", 5);
	ReadComm(ans, ansLength);

	WriteComm("AT+CMEE=1\r", 10);
	ReadComm(ans, ansLength);

	// PDU模式DEBUG_INFO: CMGF 0 used to choose PDU mode, 1 used to choose TEXT mode
	WriteComm("AT+CMGF=0\r", 10);
	ReadComm(ans, ansLength);
	// can not set mode as PDU mode.
	if(strstr(ans, "OK") == NULL)
		return FALSE;
...
int gsmSendMessage(SM_PARAM* pSrc)
{
...
#ifndef USE_TRAFFIC
	memset(pdu, 0x00, ansLength); //pdu串
	memset(ans, 0x00, ansLength); //应答串
	...
	nLength = ReadComm(ans, ansLength);	// 读应答数据
	Sleep(2000);
	...
	if(strstr(ans, "OK") == NULL)
	{
		...
		WriteComm(pdu, strlen(pdu));
		Sleep(10000);
		nLength = ReadComm(ans, ansLength);
	}
...
{	
	...
	EnterCriticalSection(&cs); //进入临界区
#endif
	int ret = WriteComm("AT+CMGL\r", 8);
#ifndef USE_TRAFFIC
	LeaveCriticalSection(&cs);
#endif
	return ret;
}
int gsmReadMessageList(int index)
{
	char cmd[16];		// 命令串
#ifndef USE_TRAFFIC
	EnterCriticalSection(&cs);
#endif
	sprintf(cmd, "AT+CMGL=%d\r", index);	// 生成命令
	int ret = WriteComm(cmd, strlen(cmd));
...
#endif
	// 从串口读数据，追加到缓冲区尾部
	nLength = ReadComm(&pBuff->data[pBuff->len], ansLength);	
	pBuff->len += nLength;
