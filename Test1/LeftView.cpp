...
CTest1Doc * CLeftView::GetDocument() // 非调试版本是内联的
{
...
void CLeftView::OnInitialUpdate()
{
	...
	for(int i =0 ; i<12; i++){
		...
		if(Table[i].chineseName.Find(_T("关系")) == -1 && Table[i].lpfCreateDlg != NULL)
...
void CLeftView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if(temp.Find(_T("添加")) == -1){
		...
		if(GetTreeCtrl().GetParentItem(hItem) == NULL && GetTreeCtrl().ItemHasChildren(hItem) == 0&&temp.Find(_T("关系")) == -1&&Table[index].lpfCreateDlg != NULL){
